...
import { createObservation, updateObservation } from "./create_update_observation";
...
function extractObservationId(endpoint: string): number {
    ...
    throw new Error(`Could not extract observation ID from endpoint: ${endpoint}`);
}
    ...
    while (true) {
        ...
        try {
            ...
            if (attemptCount >= MAX_RETRY_ATTEMPTS) {
                ...
                console.error(`Max retry attempts reached for request ID ${request.data.field_data.observation_id}.`);
